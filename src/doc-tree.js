...
function scanDir(baseDir, relDir, options) {
  relDir = relDir || '';
  options = options || {};
  ...
  for (const entry of entries) {
    ...
    if (options.excludedNames && options.excludedNames[entry.name]) continue;
    ...
    if (entry.isDirectory()) {
      dirs.push({
        ...
        type: 'dir',
        children: scanDir(baseDir, path.join(relDir, entry.name), options),
      });
