(function () {
  ...
  async function initRoots() {
    try {
      ...
      if (!resp.ok) throw new Error('No API structure');
      rootData = await resp.json();
    } catch (_e) {
      try {
        var resp = await fetch('structure.json');
        if (!resp.ok) throw new Error('No static structure');
        rootData = await resp.json();
      } catch (_e2) { rootData = []; }
    }
  ...
  async function navigate() {
    ...
    var subPath = parsed.sub.endsWith('.md') ? parsed.sub : parsed.sub + '.md';
    var docPath = (rootNames.length > 1 && parsed.root ? parsed.root + '/' : '') + subPath;
  ...
  async function findFirstDoc(rootName) {
    try {
      var resp;
      try {
        resp = await fetch('/api/structure');
        if (!resp.ok) throw new Error('No API structure');
      } catch (_apiErr) {
        resp = await fetch('structure.json');
        if (!resp.ok) throw new Error('No static structure');
      }
